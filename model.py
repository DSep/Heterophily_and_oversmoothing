...
class GCNConvolution(nn.Module):
    ...
    def __init__(self, in_features, out_features, num_vnodes=0, first_layer=False, bias=True, learn_feats=False):
...
class GCN(nn.Module):
    ...
    def __init__(self, nfeat, nlayers, nhid, nclass, dropout, learn_feats=False, num_vnodes=218, is_embed=False):
        ...
        if self.is_embed:
            ...
            # self.embedding.append(nn.Linear(nfeat, 256))
            # self.embedding.append(nn.Linear(256, nhid))
            # Linear embedding
